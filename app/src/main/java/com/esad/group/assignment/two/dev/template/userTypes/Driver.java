...
public class Driver extends SignUpUserNetwork {
    // driver registration
    @Override
    public void network(Context context, Object object, RegisterUser registerUser) {
        ServiceLocator.getInstance().getApi(ServiceLocator.GSON).createDriver(registerUser).enqueue(new Callback<RegisterUserResponse>() {
            @Override
            public void onResponse(Call<RegisterUserResponse> call, Response<RegisterUserResponse> response) {
                try {
                    if (response.isSuccessful() && response.body() != null) {
                        Log.d("LOG", "Successfully registered : " + response.body().toString());
                        SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context);
                        SharedPreferences.Editor editor = sharedPreferences.edit();
                        editor.putInt(AppConstants.DRIVER_SP, response.body().getPassengerId());
                        editor.commit();
